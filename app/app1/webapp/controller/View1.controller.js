sap.ui.define([
    ...
    return Controller.extend("app1.controller.View1", {
            ...
            var oDialog = new sap.m.Dialog({
                ...
                buttons: [
                    new sap.m.Button({
                        ...
                        type: "Emphasized",
                        press: () => {
                            this.sendInvoiceForApproval(invoiceData);
                            oDialog.close();
                        }
                    }),
                    new sap.m.Button({
        ...
        sendInvoiceForApproval: function (invoiceData) {
            debugger
            var oModel = this.getOwnerComponent().getModel();
        
            try {
                var oFunction = oModel.bindContext("/SendForApproval(...)");
                oFunction.setParameter("invoiceId", invoiceData.invoiceNumber);
                oFunction.execute().then(() => {
                    sap.m.MessageToast.show("Invoice sent for approval.");
                }).catch((error) => {
                    sap.m.MessageToast.show("Failed to send invoice for approval.");
                    console.error("❌ Error sending invoice for approval:", error);
                });
            } catch (error) {
                sap.m.MessageToast.show("Unexpected error while sending invoice for approval.");
